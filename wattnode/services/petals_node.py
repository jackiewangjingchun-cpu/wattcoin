"""
WSI Petals Node Service — manages a Petals server process for WattNode.
...
module handles starting/stopping the Petals server that hosts model layers.

Usage (standalone test):
    python petals_node.py status     # Check GPU + Petals availability
    python petals_node.py serve      # Start serving model blocks
    python petals_node.py stop       # Stop serving

Usage (from WattNode GUI):
    from services.petals_node import PetalsNodeService
    svc = PetalsNodeService(config)
    svc.check_system()      # → system report
    svc.install()           # → pip install petals
    svc.start_serving()     # → launch Petals server
...
logger = logging.getLogger("petals-node")
...
DEFAULT_PORT = 31330        # Petals server default P2P port
...
STATUS_FILE = os.path.expanduser("~/.wattnode/petals_status.json")


class PetalsNodeService:
    """
    Manages a Petals server process for serving model inference blocks.
    ...
    def check_system(self):
        ...
        report = {
            ...
            "petals_installed": self._check_petals_installed(),
        ...
        if gpu_ok and ram_ok and disk_ok:
            report["overall"] = "ready" if report["petals_installed"] else "needs_install"
        elif not gpu_ok:
    ...
    def _check_petals_installed(self):
    ...
    def install(self, progress_callback=None):
        ...
        steps = [
            ...
            ("Petals (distributed inference)", "pip install petals"),
    ...
    def start_serving(self):
        ...
        self._emit_log(f"🚀 Starting Petals server...")
        ...
        try:
            ...
            if self.on_status_change:
                self.on_status_change("starting", "Petals server launching...")
            ...
            err = "Petals not installed. Run setup first."
            self._error = err
            self._status = "error"
    ...
    def stop_serving(self):
        """Stop the Petals server."""
        if not self._running or not self._process:
            self._running = False
            self._status = "stopped"
            return {"success": True, "message": "Not running"}

        self._emit_log("🛑 Stopping Petals server...")
        ...
        try:
            ...
            if self.on_status_change:
                self.on_status_change("stopped", "Petals server stopped")
    ...
    def _read_logs(self):
        """Read Petals server output in background thread."""
        try:
            for line in self._process.stdout:
                line = line.rstrip()
                self._emit_log(line)

                # Detect status changes from Petals output
                lower = line.lower()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="WSI Petals Node Service")
