"""
WSI Petals Gateway — HTTP server that wraps the Petals distributed inference client.
Runs on the seed node (or any node with Petals client installed).
Accepts HTTP requests from the Railway API and routes them through the Petals swarm.
...
Usage:
    python petals_gateway.py                          # default port 8090
    python petals_gateway.py --port 8090 --host 0.0.0.0
    WSI_GATEWAY_KEY=mysecret python petals_gateway.py # with auth

Requirements:
    pip install petals torch transformers flask
...
def load_model(model_name):
    """
    Load a Petals distributed model + tokenizer.
    This connects to the Petals swarm and finds nodes hosting the model layers.
    ...
    logger.info(f"Loading model: {model_name} (connecting to Petals swarm...)")
    ...
    try:
        ...
        err = "Petals not installed. Run: pip install petals torch transformers"
...
def run_inference(prompt, model_name=None, max_tokens=500, temperature=0.7):
    """
    Run distributed inference through the Petals swarm.
    ...
    try:
        ...
        # In multi-node, Petals routes internally but doesn't expose per-node stats easily
...
def swarm_endpoint():
    ...
    # Try to get Petals swarm info if available
    swarm_nodes = []
    try:
        # Petals exposes some DHT info — model-specific
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="WSI Petals Gateway")
    ...
    print("=" * 60)
    print("  WSI Petals Gateway v1.0.0")
